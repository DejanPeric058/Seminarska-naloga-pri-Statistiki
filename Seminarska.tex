...
$$ \Big(\hat{\mu} - \hat{SE}_+ \Phi^{-1}(1-\frac{\alpha}{2}), \quad 
\hat{\mu} + \hat{SE}_+ \Phi^{-1}(1-\frac{\alpha}{2})\Big) \text{,}
$$

kjer je $\Phi^{-1}$ inverz komulativne funkcije normalne porazdelitve, $\hat{SE}$
pa je srednja kvadratična napaka, definirana kot

$$ \hat{SE}_+ = \frac{\hat{\sigma}}{\sqrt{n}} \text{.}
$$
...
$$ \Big(\hat{\mu} - \hat{SE}_+ \cdot F_{Student(n-1)}^{-1}(1-\frac{\alpha}{2}), \quad 
\hat{\mu} + \hat{SE}_+ \cdot F_{Student(n-1)}^{-1}(1-\frac{\alpha}{2})\Big) \text{,}
$$
